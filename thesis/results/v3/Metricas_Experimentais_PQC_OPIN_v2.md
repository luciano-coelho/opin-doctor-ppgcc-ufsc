...
| mTLS_handshake_bytes — tamanho do handshake P50 | 9.785 bytes (constante em todos os cenários) | 19.756 bytes (todos cenários) | +102% | Parâmetro que cresce com os certificados ML-DSA-65 do cliente e do gateway (Experimento 2 completo — ver nota no Bloco B). Perfeitamente estável nos dois experimentos agora que o login é totalmente automatizado — evidência de que o tamanho do handshake é determinístico para os dois algoritmos, não só para o PQC. Maior em bytes, porém mais rápido em tempo: ver nota logo após a tabela de latência por endpoint. |
