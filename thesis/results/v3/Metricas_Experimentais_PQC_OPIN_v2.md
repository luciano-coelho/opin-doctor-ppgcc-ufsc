...
Este documento apresenta dois números diferentes chamados de "OPINsize", e é importante não confundi-los: o OPINsize analítico (95.232 bytes clássico / 264.374 bytes PQC, crescimento 2,78x, Bloco C) e o OPINsize empírico por participante (~67.604 bytes clássico / ~184.637 bytes PQC, crescimento 2,73x, tabela abaixo). Os dois coexistem porque respondem perguntas diferentes e complementares, não porque um deles esteja errado.
...
O Bloco A mostrou que o fluxo fica 2,73 vezes mais pesado em bytes com PQC. Mas bytes maiores não significam necessariamente uma aplicação mais lenta — isso depende de quanto tempo o servidor gasta processando cada operação criptográfica.
...
Com os parâmetros reais do Experimento 1 (N_mTLS constante de 6 conexões e handshake P50 de 9.785 bytes, ambos idênticos em todos os 6 cenários): OPINsize clássico = 6 × 9.785 + 26 × 1.385 + 2 × 256 = 58.710 + 36.010 + 512 = 95.232 bytes.
...
Crescimento analítico: 264.374 / 95.232 = 2,78x. Este é o overhead da migração PQC pura medido pela equação analítica, usando os parâmetros empíricos dos experimentos.

**Nota — a correção do N_mTLS revelou que o valor analítico anterior estava distorcido, e agora converge com o empírico.** Até esta atualização, N_mTLS clássico era medido como uma média (~9 a 16 conexões, média ≈ 11) porque `opin_flow.py` abria um navegador real do sistema operacional para a etapa de login manual, e esse navegador tentava suas próprias conexões mTLS contra o gateway — tentativas que nunca fizeram parte do fluxo de consentimento em si, e cujo sucesso ou falha dependia de condições de corrida não determinísticas do navegador (ver DECISIONS.md, Decisão 13). Depois de automatizar completamente o login (eliminando o navegador e a espera manual), N_mTLS caiu para um valor constante de 6 conexões nos dois experimentos, e o handshake_bytes P50 do lado clássico também se estabilizou em 9.785 bytes em todos os 6 cenários — sem mais precisar de média excluindo o cenário 0ms, como a versão anterior deste documento fazia. O efeito líquido é que o OPINsize analítico clássico caiu de 151.120 para 95.232 bytes, e o crescimento analítico subiu de 1,75x para 2,78x — muito mais próximo do crescimento empírico já registrado no Bloco A (2,73x). Essa convergência entre dois métodos de medição independentes, a equação analítica (que inclui o custo do handshake mTLS) e a soma empírica por participante (que mede só a camada de aplicação), é evidência de consistência interna: o valor antigo (1,75x) estava distorcido pelo ruído do navegador, o novo (2,78x) não.
...
|---|---|---|---|---|
| OPINsize total (bytes) | ~67.604 | ~184.637 | 2,73x | O fluxo completo de consentimento OPIN fica 2,73 vezes mais pesado com PQC puro. Este é o custo da substituição completa dos algoritmos clássicos por ML-DSA-65, incluindo a cadeia de certificados PKI/CRL agora simulada localmente (Decisão 12). Estima-se que o Experimento 3 (híbrido) supere este valor — ao rodar PS256 e ML-DSA-65 em paralelo, cada JWT acumularia as duas assinaturas simultaneamente, tornando o fluxo híbrido potencialmente mais pesado que o PQC puro. O experimento confirmará ou corrigirá essa estimativa. |
| Handshake mTLS — P50 (bytes) | 9.785 | 19.756 | +102% | Antes de qualquer troca de dados, os participantes precisam estabelecer uma conexão segura. Esse processo de autenticação custa 102% mais em bytes com PQC — o cliente e o servidor (gateway) apresentam certificados ML-DSA-65, cada um contribuindo com uma assinatura maior que a RSA equivalente. O certificado digital que o cliente apresenta para se identificar cresceu de 1.494 bytes (clássico) para 2.953 bytes (ML-DSA-65). |
...
| Bytes PKI/CRL | 10.668 | 17.276 | +61,9% | Os certificados da Autoridade Certificadora (root-ca.pem/issuer-ca.pem) agora são simulados localmente com ML-DSA-65 para completar a cadeia de certificados sob comparação com a literatura (Decisão 12 do DECISIONS.md). Esta é uma simulação local para fins de medição de custo — em produção, essa migração dependeria da Raidiam atualizar sua PKI real, infraestrutura externa fora do controle desta tese. |

| Endpoint | Clássico P50 (0ms rede) | PQC P50 (0ms rede) | Overhead computacional | O que isso significa |
|---|---|---|---|---|
| /token (POST) | 21,25ms | 68,54ms | +47,29ms (+223%) | O /token é o endpoint onde o cliente prova sua identidade para o servidor. Para fazer isso, ele cria um token especial chamado client assertion. O servidor recebe esse token, verifica a assinatura e, se tudo estiver certo, devolve o token de acesso. Com PS256, criar e verificar essa assinatura leva cerca de 21,25ms. Com ML-DSA-65, o mesmo processo leva 68,54ms. A diferença de 47,29ms é o tempo extra que o algoritmo pós-quântico consome. |
...
| /jwks (GET) | 39,64ms | 46,07ms | +6,43ms (+16%) | O endpoint /jwks é onde o cliente busca a chave pública do servidor para verificar os tokens que recebe. É uma operação simples de leitura — o servidor não faz nenhum cálculo criptográfico, só devolve a chave pública que já está armazenada. Como não há operação de assinatura ou verificação nesse endpoint, o tempo de resposta é praticamente equivalente entre os dois algoritmos. |

| Parâmetro | Clássico (Exp 1) | PQC Puro (Exp 2) | Crescimento | O que isso significa para a equação OPINsize |
|---|---|---|---|---|
| N_mTLS — número de handshakes distintos | 6 (constante em todos os cenários) | 6 (constante em todos os cenários) | Estável | O número de conexões não muda com o algoritmo — só o custo de cada uma. Com o login totalmente automatizado (Decisão 13 do DECISIONS.md, que eliminou o navegador real que o script abria para a etapa de login manual), o valor é perfeitamente constante nos dois experimentos; a variação ~9 a 16 medida anteriormente no clássico vinha das tentativas de conexão não determinísticas desse navegador, não do fluxo de consentimento medido. |
| mTLS_handshake_bytes — tamanho do handshake P50 | 9.785 bytes (constante em todos os cenários) | 19.756 bytes (todos cenários) | +102% | Parâmetro que cresce com os certificados ML-DSA-65 do cliente e do gateway (Experimento 2 completo). Perfeitamente estável nos dois experimentos agora que o login é totalmente automatizado — evidência de que o tamanho do handshake é determinístico para os dois algoritmos, não só para o PQC. |
...
|---|---|---|---|---|
| OPINsize total (bytes) | ~67.604 | ~184.637 | Estimado acima de ~184.637 | No híbrido cada JWT carrega PS256 e ML-DSA-65 simultaneamente. O OPINsize deve superar o PQC puro pelo acréscimo das assinaturas PS256 adicionais. |
...
| Handshake mTLS P50 (bytes) | 9.785 | 19.756 | Estimado próximo ou acima de 19.756 | O certificado híbrido carregará as duas assinaturas. O handshake deve ser similar ao PQC puro ou ligeiramente maior. |
