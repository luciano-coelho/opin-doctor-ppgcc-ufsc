...
Tamanhos resultantes por fluxo (idênticos em todos os cenários de latência): certificado de cliente de 1.494, 2.953 e 6.859 bytes; JWT médio de 1.385,42, 5.458,81 e 7.324,81 bytes; handshake de 5.119, 16.605 e 18.023 bytes (Clássico, PQC, Híbrido).
...
**O que a tabela abaixo mostra.** Para cada perfil, o tamanho em bytes do certificado raiz e do certificado da emissora (no formato PEM, como trafegam), e a soma dos 4 downloads que compõem o termo de PKI da equação — a base numérica de tudo que a Seção 7.2 discute.
...
- **Enquadramento HTTP.** Usando diretamente o volume de resposta medido (corpo + 103 bytes por certificado), o OPINsize muda em menos de 1 ponto percentual — a tabela abaixo é a mesma comparação, agora com o termo de PKI medido pelo tráfego HTTP real em vez do tamanho do arquivo PEM:
