# Relatório Consolidado de Métricas Experimentais — FINAL
...
| Servidor de Autorização — AS | 14.188 | 29.570 | 31.058 | Volume de saída do AS (consentimento, PAR, tokens, JWKS). PQC é 2,08× o Clássico (+108,42%); Híbrido é 2,19× o Clássico (+118,90%) e 1,05× o PQC (+5,03%) — o AS emite proporcionalmente menos JWTs pesados que o RS, então seu crescimento relativo é mais modesto. |
...
## 5. Consolidação Arquitetural dos Três Perfis

As tabelas das Seções 2 a 4 respondem "quanto"; esta seção responde "por quê" — o que cada perfil faz, de forma diferente, para produzir esses números. Não é preciso ler `ARCHITECTURE.md` inteiro para entender a origem dos valores: um resumo objetivo de cada perfil está aqui, artefato por artefato.

### 5.1 O que muda, artefato por artefato

A tabela abaixo é a referência central: para cada artefato criptográfico do fluxo (uma linha), o algoritmo ou esquema usado em cada perfil (uma coluna). Ela é a base de todas as métricas de tamanho e latência apresentadas nas Seções 2 a 4 — por exemplo, a linha "JWT de resposta do RS" explica por que o JWT médio salta de 1.385 para 5.458 bytes (Seção 2): a assinatura PS256 (256 bytes) é trocada pela ML-DSA-65 (muito maior) no PQC, e as duas coexistem no mesmo token no Híbrido.

Cada célula linka o artefato correspondente, capturado ao vivo naquele perfil, para quem quiser conferir diretamente (`thesis/results/v7/artifacts/`) — o arquivo bruto e, quando existe, a checagem criptográfica independente rodada contra ele.

| Artefato | Clássico | PQC | Híbrido |
|---|---|---|---|
| Troca de chave TLS | ECDHE clássico (P-521/P-384/P-256) — [artefato](artifacts/classico/handshake_log.json) · [verificação](artifacts/classico/verify_kem_export_output.txt) | **ML-KEM-1024** puro — [artefato](artifacts/pqc/handshake_log.json) · [verificação](artifacts/pqc/verify_kem_export_output.txt) | **X25519MLKEM768** — [artefato](artifacts/hybrid/handshake_log.json) · [verificação](artifacts/hybrid/verify_kem_export_output.txt) |
| Certificado de cliente (mTLS) | RSA-4096, assinado por CA RSA — [certificado](artifacts/classico/client_one.crt.txt) · [verificação](artifacts/classico/verify_classic_cert_output.txt) | Chave do titular ML-DSA-65 — [certificado](artifacts/pqc/client_one_pqc.crt.txt) · [verificação](artifacts/pqc/verify_pqc_cert_output.txt) | RSA-4096 + extensões X.509 com o material ML-DSA-65, assinado duas vezes — [certificado](artifacts/hybrid/client_one_hybrid.crt.txt) · [verificação](artifacts/hybrid/verify_hybrid_cert_output.txt) |
| Certificado de CA (PKI/CRL) | RSA | ML-DSA-65 (titular), assinado por CA RSA (simulação local) | Duas assinaturas (RSA + ML-DSA-65), mesma lógica do certificado de cliente — sem captura em `artifacts/`; arquivos em `mock-service-os/certs/` (`root_ca*.crt`, `issuer_ca*.crt`), discussão completa na Seção 5.3 abaixo e em `ARCHITECTURE.md`, Seção 7.1 |
| JWT de resposta do RS | PS256 — [artefato](artifacts/classico/example_jwt_premium.txt) · [verificação](artifacts/classico/verify_jwt_output.txt) | ML-DSA-65 — [artefato](artifacts/pqc/example_jwt_premium.txt) · [verificação](artifacts/pqc/verify_jwt_output.txt) | RS256 com extensão de payload carregando a assinatura ML-DSA-65 — [artefato](artifacts/hybrid/example_jwt_premium.txt) · [verificação](artifacts/hybrid/verify_jwt_output.txt) |
| `id_token` e JARM do AS | PS256 — [artefato](artifacts/classico/id_token_raw.txt) · [verificação](artifacts/classico/verify_id_token_output.txt) | ML-DSA-65 — [artefato](artifacts/pqc/id_token_raw.txt) · [verificação](artifacts/pqc/verify_id_token_output.txt) | Strong Nesting (assinatura clássica ‖ assinatura pós-quântica) — [artefato](artifacts/hybrid/id_token_raw.txt) · [verificação](artifacts/hybrid/verify_id_token_output.txt) |
| `client_assertion` e objeto PAR | PS256 — [artefato](artifacts/classico/client_assertion_raw.txt) · [verificação](artifacts/classico/verify_client_assertion_output.txt) | ML-DSA-65 — [artefato](artifacts/pqc/client_assertion_raw.txt) · [verificação](artifacts/pqc/verify_client_assertion_output.txt) | RS256 com extensão de payload — [artefato](artifacts/hybrid/client_assertion_raw.txt) · [verificação](artifacts/hybrid/verify_client_assertion_output.txt) |
| Chave pública do AS (JWKS) | RSA, 256 bytes — [artefato](artifacts/classico/jwks_auth.json) | ML-DSA-65, 1.952 bytes — [artefato](artifacts/pqc/jwks_auth.json) | Chave composta (`kty: HYBRID`), 2.208 bytes — [artefato](artifacts/hybrid/jwks_auth.json) |

("artefato" aponta para o arquivo bruto capturado ao vivo; "verificação" aponta para a checagem criptográfica independente rodada contra esse mesmo arquivo — ambos em `thesis/results/v7/artifacts/<perfil>/`, com a metodologia completa de captura em cada `README.md`.)

### 5.2 Clássico — a linha de base

Nenhuma primitiva pós-quântica em nenhum artefato: é o comportamento que o ecossistema OPIN já tinha antes desta tese, contra o qual PQC e Híbrido são comparados em todas as tabelas anteriores.

### 5.3 PQC puro — substituição completa, com uma exceção deliberada

Assinatura e troca de chave sem nenhum componente clássico, exceto num ponto, o mesmo em todo certificado do perfil (do cliente e da PKI/CRL): a **chave do titular** migra para ML-DSA-65, mas a **assinatura do emissor** (a CA que emite o certificado) continua RSA. Por que não migrar a CA também? Duas razões distintas, uma de escopo metodológico e outra de fato de infraestrutura — nenhuma das duas é uma limitação encontrada por acidente, ambas foram decisões deliberadas:

1. **Escopo: a tese mede a migração da identidade do participante, não da hierarquia de confiança inteira.** Migrar uma CA de verdade significa gerar uma nova cadeia de confiança e fazer cada parte relying party reconhecê-la — uma operação de coordenação entre organizações, ortogonal ao que esta tese mede (o custo de um fluxo OIDC individual). Isolar "só a identidade do titular migra" deixa a CA como uma variável controlada, permitindo medir o efeito de uma primitiva de cada vez. Essa decisão foi tomada e documentada como **"Etapa 3.1"** durante a implementação do Experimento 2 (`thesis/results/v2/experiment2 - PQC/DECISIONS.md`, Decision 5): o certificado do titular é gerado com uma chave ML-DSA-65 e assinado pela CA RSA já existente no ambiente — uma assinatura de emissor clássica sobre uma chave pública de titular pós-quântica é X.509 válido e ordinário (os dois campos são independentes), e reemitir a CA teria invalidado todos os outros certificados do ambiente assinados por ela.
2. **Fato de infraestrutura: para o par `root-ca.pem`/`issuer-ca.pem` especificamente, a CA real é da Raidiam** (`crl.sandbox.pki.opinbrasil.com.br`), fora do controle desta tese — não é um serviço deste projeto para migrar. Os arquivos servidos no perfil PQC (`root_ca_pqc.crt`/`issuer_ca_pqc.crt`) são simuladores locais, gerados só para que a métrica de bytes da PKI/CRL reflita o tamanho de um certificado pós-quântico — não uma alegação de que a PKI real da Raidiam foi migrada. Essa decisão (`thesis/results/v2/experiment2 - PQC/DECISIONS.md`, Decision 12) só foi tomada depois de confirmar, lendo o código (`opin_flow.py`, `fetch_server_keys_and_ca()`; `mock_mtls/main.go`, `caCertPool()`), que nenhuma outra parte do fluxo depende do conteúdo criptográfico real desses dois arquivos — o gateway nunca usa `root-ca.pem`/`issuer-ca.pem` para validar nada, só os serve; então simular o tamanho não corrompe nenhuma outra métrica.

Essa exceção explica um achado da Seção 3: o termo de PKI representa uma fatia menor do OPINsize no PQC (6,19%) do que no Híbrido (10,98%) — no PQC, só a chave do titular do certificado de CA cresce; a assinatura da CA em si permanece do tamanho clássico. No Híbrido, ao contrário, o certificado de CA carrega as duas assinaturas (Seção 5.4), por isso cresce mais.

### 5.4 Híbrido — combinação, não substituição

Esse trabalho foi concluído na v7: em vez de escolher um único algoritmo por variável de configuração, o AS e o RS agora assinam com os dois algoritmos simultaneamente quando `CRYPTO_PROFILE=hybrid`, e o certificado do cliente e do gateway carregam as duas assinaturas ao mesmo tempo — por isso o Híbrido é sempre maior e mais lento que o PQC puro em todas as métricas deste documento: ele não troca uma assinatura por outra, soma as duas.

Três esquemas de combinação são usados, um por tipo de artefato, cada um escolhido pela propriedade de segurança mais adequada ao seu papel:

| Esquema | Onde | Propriedade |
|---|---|---|
| Extensões X.509 duplamente assinadas (Bindel et al., 2019) | Certificados (cliente e CA) | Compatibilidade com verificadores legados, que ignoram a extensão não crítica |
| Extensão de payload | JWT do RS, `client_assertion`, objeto PAR | Um verificador RS256 comum aceita o token ignorando o claim extra |
| Strong Nesting (Bindel et al., 2017) | `id_token`, JARM do AS | Propriedade SUF-CMA: não é possível recombinar uma assinatura clássica com outra pós-quântica |

A troca de chave híbrida usa o grupo `X25519MLKEM768`: a chave de sessão deriva do X25519 e do ML-KEM-768 juntos, preservando a confidencialidade se apenas um dos dois for quebrado. O detalhamento de cada esquema, com as referências completas, está em `ARCHITECTURE.md`, Seção 4.2.

Nenhuma métrica nova precisou ser criada para medir o Híbrido: OPINsize, tamanho do handshake mTLS, tamanho médio de JWT, bytes por participante e latência por endpoint são as mesmas métricas já definidas para Clássico e PQC, medidas pelo mesmo pipeline de coleta e análise — as mesmas que aparecem em todas as tabelas deste documento.

## 6. Proveniência dos números

Todos os valores desta tabela vêm de `thesis/results/v7/report_data_v7.json`, computado por `thesis/scripts/compute_v7_report_data.py` a partir dos arquivos brutos das 180 execuções oficiais (`thesis/results/v7/size/` e `thesis/results/v7/latency/`), e recalculados de forma independente antes da consolidação (`thesis/scripts/audit_v7_from_raw.py`, saída completa em `audit_recompute_from_raw.txt`). 
